namespace Bank.Services.DomainServices.Loan
{
    ...
    public class VipLoanDomainService : ILoanDomainService
    {
        public bool CanCreateLoan(Loan loan, Customer customer, List<Loan> customerLoans)
        {
            ...
            if (customer.MonthlySalary / 2 < totalMonthlyLoanAmount)
            {
                var errorMessage = $"Loan amount is too big. Maximum loan amount is {customer.MonthlySalary / 2} per month. " +
                                   $"Your provided loan amount per month (with interest rate) is {totalMonthlyLoanAmount} per month.";

                throw new BusinessException(errorMessage);
            }

            if (customerLoans.Count > 2)
            {
